...
CONSTRAINT fkAdmin FOREIGN KEY (administrador) REFERENCES Empresa (idEmpresa)
);
...
fkVeiculo INT,
fkMotorista INT,
idFrota INT PRIMARY KEY AUTO_INCREMENT,
CONSTRAINT fkVeiculo FOREIGN KEY (fkVeiculo) REFERENCES Veiculo (idVeiculo),
CONSTRAINT fkMotorista FOREIGN KEY (fkMotorita) REFERENCES Motorista (idMotorista)
);
...
SELECT * FROM Empresa Join Usuario ON fktoken = token;
